...
```bash
docker build -t pi/face_detection -f dockerfile .
```
...
```bash
docker run -it --rm -p8080:8080 pi/face_detection
```

or with custom environment variables:

```bash
docker run -it --env APP_HOST=0.0.0.0 --env APP_PORT=4040 --env APP_LOG=info --env APP_... --rm -p8080:4040 pi/face_detection
```
