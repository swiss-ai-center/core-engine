...
if __name__ == "__main__":
    ...
    PATH_GRAPH = "./graphs"
    # Load data from CSV
    ftrain = f"{PATH_DATASET}/train.csv"
    ftest = f"{PATH_DATASET}/test.csv"
    ...
    plt.xlabel('epoch')
    plt.legend()
    plt.savefig(f"{PATH_GRAPH}/model_accuracy.jpg")
    plt.clf()
    ...
    plt.xlabel('epoch')
    plt.legend()
    plt.savefig(f'{PATH_GRAPH}/model_loss.jpg')
    plt.clf()
    ...
    model.save(f'{PATH_MODELS}/mnist_model.h5')
