...
import { setColorMode } from './utils/reducers/colorModeSlice';
...
function App() {
  ...
  const currentColorMode = systemColorModeToLightOrDark(useSelector((state: any) => state.colorMode.value));
  ...
  const handleColorMode = () => {
    let newColorMode;
    if (colorMode === 'light') newColorMode = 'dark';
    else if (colorMode === 'dark') newColorMode = 'system';
    else newColorMode = 'light';
  ...
  return (
    <ThemeProvider theme={theme}>
      ...
      <AppBar position={"fixed"} elevation={2} sx={{
        ...
        backgroundColor: currentColorMode === "light" ? "primary.main" : "background_color.main"
      }}>
        <Toolbar>
          {menuIcon ?
            <IconButton
          ...
          <Grid container justifyContent={"space-between"} alignItems={"center"} sx={{height: "100%", width: "100%"}}
            ...
            <Grid>
              <Link color={"inherit"} href={"/"} underline={"none"}>
                <img src={currentColorMode === "light" ? "/logo_full_white_sec.png" : "/logo_full_white.png"}
            ...
            <Grid>
              <Tooltip title={"Toggle dark / light mode"}>
                <IconButton sx={{marginLeft: "auto"}} color={"inherit"} size={"large"}
                  ...
                  {colorMode === 'light' ? <LightModeTwoTone/> : colorMode === 'dark' ? <DarkModeTwoTone/> :
                    <BrightnessAutoTwoTone/>}
                </IconButton>
              </Tooltip>
