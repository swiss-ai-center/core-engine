...
import { positionHandle, systemColorModeToLightOrDark } from "utils/functions";
...
const EntryNodeEdit: React.FC<{ id: string, data: any }> = (
  ...
  const dataInSelection = data.dataIn.map((inputField: { name: string; type: string[]; }, index: number) =>
    <Grid sx={{display: "flex", flexGrow: 1}} key={inputField.name} mb={index === data.dataIn.length - 1 ? 0 : 1}
          spacing={2} container>
      <Grid key={`${inputField.name}-name`} size={{xs: 5}}>
      ...
      <Grid key={index} sx={{display: "flex", alignItems: "center", justifyContent: "center"}} size={{xs: 1}}>
        <IconButton
