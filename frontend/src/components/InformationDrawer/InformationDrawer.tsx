...
import './styles.css';
import { isSmartphone, systemColorModeToLightOrDark } from 'utils/functions';
    ...
    <>
      ...
      <Box m={isSmartphone() ? 2 : 3}
        ...
        <Box sx={{mb: isSmartphone() ? 2 : 3}}>
          ...
          <Grid container mb={3}>
            <Grid size={{xs: description.has_ai ? 11 : 12, sm: description.has_ai ? 10 : 12}}>
              <Grid container>
            ...
            {description.has_ai ? (
              <Grid size={{xs: 1, sm: 2}} sx={{display: 'flex', justifyContent: 'flex-end'}}>
        ...
        <Box>
          {description ?
            <>
              <Accordion variant={"outlined"} color={colorMode} expanded>
                  ...
                  {description.data_in_fields ? description.data_in_fields.map((data: any, index: number) => {
                      return (
                        <Grid key={`data-in-${index}`} container>
                          ...
                          <Grid size={{xs: 8}} justifyContent={"flex-end"} display={"flex"}>
                  ...
                  {description.data_out_fields ? description.data_out_fields.map((data: any, index: number) => {
                      return (
                        <Grid key={`data-out-${index}`} container>
                          ...
                          <Grid justifyContent={"flex-end"} display={"flex"} size={{xs: 8}}>
                            <Typography variant={"subtitle1"}>
                              {data.type.map((type: string, index: number) => {
                                return (
