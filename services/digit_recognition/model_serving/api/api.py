...
async def notifyEngine():
    try:
        res = await client.post(engine + "/services", json=interface.engineAPI())
        if res.status_code != 200:
            logging.getLogger("uvicorn").warning(
                "Failed to notify the engine, request returned " + str(res.status_code))
...
async def startup():
    ...
    # Announce ourselves to the engine
