...
class MyService(Service):
    ...
    def __init__(self):
        super().__init__(
            name="Image Convert to PNG",
            slug="image-convert-to-png",
    ...
    async def process(self, data):
        ...
        out_buff = io.BytesIO()
        img.save(out_buff, format=FieldDescriptionType.IMAGE_PNG.split('/')[1])
        out_buff.seek(0)
        return {
...
api_description = """
This service converts images from different formats to PNG.
Acceptable formats are: image/png, image/png.
"""
api_summary = """
...
app = FastAPI(
    title="Image Convert to PNG API.",
    description=api_description,
    version="0.0.1",
    contact={
        "name": "CSIA-PME",
